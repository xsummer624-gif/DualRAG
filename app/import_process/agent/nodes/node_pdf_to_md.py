...
def node_pdf_to_md(state: ImportGraphState) -> ImportGraphState:
    ...
    try:
        ...
        logger.info(f">>>[{function_name}]使用minerU发生了异常: 异常状态{e}")
        raise
    finally:
        md_len = len(state.get('md_content', ''))
        state_keys = list(state.keys())
        logger.info(f">>>[{function_name}]节点结束！状态字段: {state_keys} | md_content长度: {md_len} 字符")
